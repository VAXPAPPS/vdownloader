...
vdl_headerbar_new (GtkWindow *window)
{
    ...
    /* Left side: Extra buttons area */
    self->start_box = gtk_box_new (GTK_ORIENTATION_HORIZONTAL, 8);
    gtk_widget_set_valign (self->start_box, GTK_ALIGN_CENTER);
    gtk_widget_set_margin_start (self->start_box, 16);
    gtk_widget_set_margin_end (self->start_box, 16);
...
GtkWidget *
vdl_headerbar_get_start_box (VdlHeaderBar *self)
{
